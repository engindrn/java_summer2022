...
public class C08_OrtakElemanlariBulma {
    ...
    public static void main(String[] args) {
        ...
        List<String> ortakIsimler=new ArrayList<>(); //ortak olanları buna atacaz


        for (String each1: arr1                     //iç içe foreach loop ile yapacaz biri arr1 den diğeri arr2 den getirecek
        ) {
            for (String each2:arr2
            ) {
                if (each1.equalsIgnoreCase(each2)){
                    ortakIsimler.add(each1);              //direk yazdırabilirdik
                }
            }
        }
        if (ortakIsimler.isEmpty()){                          // hiç ortal isim yoksa
