...
public class C04_UniqueArrayOlusturma {
    ...
    public static int[] uniqueArrayOlustur(int[] arr) {
        ...
        Arrays.sort(arr);          //for loop da olur ama bu daha basit

        System.out.println(Arrays.toString(arr)); // [1, 2, 3, 3, 3, 4, 5, 5, 5, 5, 6, 6, 9]

        List<Integer> benzersizList = new ArrayList<>(); //unig elementlerin sayisini bilmediğimiz için önce list
                                                            //oluşturduk
        // yanindaki elemente esit olmayanlari listeye ekliyor

        for (int i = 0; i < arr.length-1 ; i++) {
            if (arr[i] != arr[i+1]){              //arri arr+1 e eşit değilse
                benzersizList.add(arr[i]);     // oluşturduğumuz benzersiz listine bu eşit olamayan
            }                                   //arrayin i elementini ekle
        }

        // son elementi kontrol edip, yoksa listeye ekliyoruz

        if (!benzersizList.contains(arr[arr.length-1])){    //son elementi burda kontrol ettik
            benzersizList.add(arr[arr.length-1]);          //sonuna ekledik
        }
