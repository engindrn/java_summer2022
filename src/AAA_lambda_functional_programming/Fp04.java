...
public class Fp04 {
    ...
    public static int faktoriyelHesapla(int x){
        ...
        if(x>=0){
            return IntStream.rangeClosed(1,x).reduce(Math::multiplyExact).getAsInt(); //birden başlayıp methoda parametre
        }                                                                               //olrak gönderilen sayıya kadar
    ...
    public static int verilenIkiSayiArasindakiCiftSayilarinToplamini(int x, int y){
        int z = 0;                                                  //x değeri y den küçük olmalı rangeClosed çalışması için
        if(x>y){                                        //eğer x - y den küçük değilse burda yer değişikliği yapıyoruz
            z = x;
            x = y;
            y = z;
        }
